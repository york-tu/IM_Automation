...
if __name__ == "__main__":
    ...
    suite_s1.addTests(s1_test_cases)  # total 61*s1
    suite_s2 = unittest.TestSuite()
    suite_s2.addTests(s2_test_cases)  # total 37*s2 + 3*s1

    # 紅包相關有「先建後搶」的依賴序列，群組內任一條失敗時整組重跑，避免單獨重跑必失敗
    # S1: 後台建紅包 → 前台搶；後台建拚手氣 → 前台搶
    s1_retry_groups = [
    ...
    Utils.unittest_xml_with_retry_and_slack(
        suite_s1, report_label='S1', run_check_last_result=True,
        retry_groups=s1_retry_groups,
    )
    # S2：跑完 + retry 失敗案例 → 發 S2 報告到 Slack，並執行 Jira check_last_result
    Utils.unittest_xml_with_retry_and_slack(
        suite_s2, report_label='S2', run_check_last_result=True,
        retry_groups=s2_retry_groups,
    )
