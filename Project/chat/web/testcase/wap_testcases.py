...
class WapTestCase(BaseTestCase):
    ...
    def test_all_windows_max(self):
        for function in self.function_dict.values():
            function.basePage().windows_to_top(full=True)
    ...
    def test_wap_logout(self):
        ...
        self.function_dict['wap'].wapMainPersonalPage().logout()
    ...
    def test_version_check(self):
        ...
        self.function_dict['wap'].wapMainPage().into_main_page()
        self.function_dict['wap'].wapMainPersonalPage().into_main_setting_page()
        self.function_dict['wap'].wapMainPersonalPage().into_about_page()
        self.function_dict['wap'].wapMainPersonalPage().check_version(wap_version)
    ...
    def test_about_terms(self):
        ...
        self.function_dict['wap'].wapMainPersonalPage().into_main_setting_page()
        self.function_dict['wap'].wapMainPersonalPage().into_about_page()
        self.function_dict['wap'].wapMainPersonalPage().check_service()
        self.function_dict['wap'].wapMainPersonalPage().check_privacy()
    ...
    def test_change_nickname_and_instructions(self):
        self.test_wap_login()
        self.function_dict['wap'].wapMainPage().into_main_page()
        original_nickname = self.function_dict['wap'].wapMainPage().get_nickname()
        original_descriptions = self.function_dict['wap'].wapMainPage().get_descriptions().rstrip()
        self.function_dict['wap'].wapMainPage().change_nickname('gutest1234')
        self.function_dict['wap'].wapMainPage().change_nickname(original_nickname)
        self.function_dict['wap'].wapMainPage().change_descriptions('mWebChangeDescriptionsTest')
        self.function_dict['wap'].wapMainPage().change_descriptions(original_descriptions)
    ...
    def test_friend_remark(self):
        self.test_wap_login()
        self.function_dict['wap'].wapFriendsPage().into_friend_list()
        self.function_dict['wap'].wapFriendsPage().list_friend_search(self.operate_account)
        self.function_dict['wap'].wapFriendsPage().set_remark_note('mWeb編輯好友描述Test')
        self.function_dict['wap'].wapFriendsPage().set_remark_nickname('mWeb編輯好友暱稱Test')
    ...
    def test_block_and_report_friend(self):
        self.test_wap_login()
        self.function_dict['wap'].wapMessagePage().into_chat_room(self.operate_account)
        self.function_dict['wap'].wapFriendsPage().block_friend()
        self.function_dict['wap'].wapFriendsPage().report()
    ...
    def test_unblock_friend(self):
        self.test_wap_login()
        self.function_dict['wap'].wapMainPersonalPage().into_main_setting_page()
        self.function_dict['wap'].wapMainPersonalPage().into_blocklist_page()
        self.function_dict['wap'].wapMainPersonalPage().search_blocked_friend(self.operate_account)
        self.function_dict['wap'].wapMainPersonalPage().unblock_friend()
    ...
    def test_1v1_send_message(self):
        ...
        self.function_dict['wap'].wapMessagePage().check_chatroom_list_last_message(last_text)
        # ======================= 發送"超連結"測試 ===============================================
        self.function_dict['wap'].wapMessagePage().into_chat_room(self.operate_account)
        last_url = self.function_dict['wap'].wapMessagePage().send_url_message()
        self.function_dict['wap'].wapMessagePage().check_chatroom_list_last_message(last_url)
    ...
    def test_1v1_message_copy(self):
        ...
        self.function_dict['wap'].wapMessagePage().message_copy('mWeb發訊息TeSt!@#$%_3')
        self.function_dict['wap'].wapMessagePage().check_chatroom_list_last_message('mWeb發訊息TeSt!@#$%_3')
    ...
    def test_1v1_message_reply(self):
        ...
        self.function_dict['wap'].wapMessagePage().check_chatroom_list_last_message(reply_text)
    ...
    def test_1v1_message_reply_revoke(self):
        ...
        self.function_dict['wap'].wapMessagePage().check_reply_msg('原始讯息已不存在')
    ...
    def test_1v1_message_pin_reply(self):
        ...
        self.function_dict['wap'].wapMessagePage().send_message(reply_original_message)
        self.function_dict['wap'].wapMessagePage().message_reply(reply_original_message, reply_message)
        self.function_dict['wap'].wapMessagePage().message_pin(reply_message)
        self.function_dict['wap'].wapMessagePage().delete_all_pin()
    ...
    def test_1v1_message_pin_revoke(self):
        ...
        self.function_dict['wap'].wapMessagePage().message_revoke(original_message, pin_revoke=True)
        self.function_dict['wap'].wapMessagePage().delete_all_pin()
    ...
    def test_admin_account_login(self):
        ...
        self.function_dict['wap'].basePage().windows_to_top(full=True)  # 切換視窗
        self.function_dict['wap'].basePage().open_base_url()  # 開啟wap網頁
        self.function_dict['wap'].wapLoginPage().login(self.app_phone, self.app_password, self.app_nation)
    ...
    def test_social_search(self):
        ...
        self.function_dict['wap'].searchPage().search_post(description_list[1], search_by_switch_tab=False)
        self.function_dict['wap'].searchPage().check_search_post_result(poster_nickname_list[1], description_list[1])
        # ========================== 4. 搜索視頻結果頁: 輸入01 > 點"用戶"頁籤 >>> 切換到用戶頁並立即搜索 ========================
        self.function_dict['wap'].searchPage().search_poster(poster_nickname_list[0], search_by_switch_tab=True)
        self.function_dict['wap'].searchPage().check_search_poster_result(poster_nickname_list[0])
        # ========================== 5. 搜索用戶結果頁: 輸入02 > 點搜索鍵 >>> 搜索結果-用戶頁 =================================
        self.function_dict['wap'].searchPage().search_poster(poster_nickname_list[1], search_by_switch_tab=False)
        self.function_dict['wap'].searchPage().check_search_poster_result(poster_nickname_list[1])
        # ========================== 7. 搜索用戶結果頁: 輸入02貼文說明02 > 點"視頻"頁籤 >>> 立即搜索 ===========================
        self.function_dict['wap'].searchPage().search_post(description_list[1], search_by_switch_tab=True)
        self.function_dict['wap'].searchPage().check_search_post_result(poster_nickname_list[1], description_list[1])
        # ========================== 確認搜索紀錄頁 =======================================================================
        self.function_dict['wap'].searchPage().check_recent_search_record(
    ...
    def test_social_follow_unfollow(self):
        ...
        self.function_dict['wap'].wapMainPage().into_followed_list()
        self.function_dict['wap'].wapMainPage().check_followed_list(self.operate_account, after_unfollows, add_follow=False)  # 確認關注列表: 自己關注數-1, 列表不出現對方
        # ============================ 確認對方粉絲列表 ===============================
        self.function_dict['wap'].wapMainPage().back_to_previous_page()
        self.function_dict['wap'].wapMessagePage().into_chat_room(self.operate_account)
        self.function_dict['wap'].wapFriendsPage().into_personal_profile()
