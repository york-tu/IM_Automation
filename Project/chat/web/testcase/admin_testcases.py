...
class AdminTestCase(BaseTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.setting_browser()
        admin_version = cls.function_dict['ad'].main_page().return_admin_version()  # 獲取web版本號
    ...
    def test_into_and_check_member_list(self):
        ...
        self.function_dict['ad'].member_page().check_member_list_page()

    # 測試-進入會員層級並檢查頁面基本資訊
    @DecorateClass('CHATAPP-T2740')
    def test_into_and_check_member_level(self):
        self.test_admin_login()
        self.function_dict['ad'].main_page().into_member_level()
        self.function_dict['ad'].member_page().check_member_level_page()
    ...
    def test_into_and_check_groups_list(self):
        ...
        self.function_dict['ad'].groups_page().check_group_list_page()
    ...
    def test_into_system_maintenance(self):
        self.test_admin_login()
        self.function_dict['ad'].main_page().into_system_maintenance()
    ...
    def test_into_system_app_setting(self):
        ...
        self.function_dict['ad'].main_page().check_member_add_friend_tip()  # 檢查會員添加好友選項tip
        # self.function_dict['ad'].main_page().check_add_friend_by_search_phone_tip()  # 檢查手機號搜索添加好友tip
    ...
    def test_into_system_contact_whitelist_setting(self):
        ...
        self.function_dict['ad'].base_page().windows_to_top()  # 切換視窗
        self.function_dict['ad'].base_page().open_base_url()  # 開啟前台網站
        self.function_dict['ad'].login_page().logout()
        self.function_dict['ad'].login_page().login(self.low_rights_adm_id, self.low_rights_adm_pwd)  # 登入無新增好友權限admin帳號
        assert not self.function_dict['ad'].main_page().system_contact_whitelist_setting_show()  # 確認不顯示好友添加白名单设定頁
        self.function_dict['ad'].login_page().logout()
        self.test_admin_login()  # 登入full權限admin帳號
        self.function_dict['ad'].main_page().into_system_contact_whitelist_setting() # 確認顯示好友添加白名单设定頁
    ...
    def test_into_share_code_setting(self):
        ...
        self.function_dict['ad'].base_page().open_base_url()  # 開啟前台網站
        self.function_dict['ad'].login_page().logout()
        self.function_dict['ad'].login_page().login(self.low_rights_adm_id,
                                                   self.low_rights_adm_pwd)  # 登入低權限帳號
        assert not self.function_dict['ad'].main_page().share_code_management_show()  # 確認不顯示邀請碼管理頁
        self.function_dict['ad'].login_page().logout()
        self.test_admin_login()  # 登入full權限admin帳號
        self.function_dict['ad'].main_page().into_share_code()  # 確認進入邀請碼管理頁
    ...
    def test_add_share_code(self):
        ...
        self.test_admin_login()
        self.function_dict['ad'].main_page().add_share_code(share_code_sample, 'AutoTestShareCode')
        self.function_dict['ad'].main_page().delete_share_code(share_code_sample)
    ...
    def test_into_discover_and_edit(self):
        ...
        self.function_dict['ad'].main_page().disable_discover_function(3)
        self.function_dict['ad'].main_page().restore_discover_function()
        self.function_dict['ad'].main_page().edit_restore_discover_index0_url()
    ...
    def test_into_record_check_chat_record(self):
        ...
        self.function_dict['wp'].chatroom_page().send_message(random_number)

        self.test_admin_login()
        self.function_dict['ad'].main_page().into_menu_recode()
        self.function_dict['ad'].record_page().check_chat_record(self.web_account, random_number)
    ...
    def test_into_media_audit(self):
        self.test_admin_login()
        self.function_dict['ad'].social_management_page().into_media_audit_page(first_enter=True)

    # 測試-進入自动审核
    @DecorateClass('CHATAPP-T2712')
    def test_into_auto_audit(self):
        self.test_admin_login()
        self.function_dict['ad'].social_management_page().into_auto_audit_page(first_enter=True)

    # 測試-進入屏蔽字詞
    @DecorateClass('CHATAPP-T2850')
    def test_into_block_words(self):
        self.test_admin_login()
        self.function_dict['ad'].social_management_page().into_block_words_page(first_enter=True)

    # 測試-進入检举内容
    @DecorateClass('CHATAPP-T2851')
    def test_into_impeach(self):
        self.test_admin_login()
        self.function_dict['ad'].social_management_page().into_impeach_page(first_enter=True)

    # 測試-進入贴文数据
    @DecorateClass('CHATAPP-T2852')
    def test_into_post_data(self):
        self.test_admin_login()
        self.function_dict['ad'].social_management_page().into_post_data_page(first_enter=True)

    # 測試-進入创作者数据
    @DecorateClass('CHATAPP-T2853')
    def test_into_creator_data(self):
        self.test_admin_login()
        self.function_dict['ad'].social_management_page().into_creator_data_page(first_enter=True)
    ...
    def test_build_group_permission(self):
        ...
        for _bool in bool_list:
            self.test_admin_login()
            self.function_dict['ad'].main_page().into_groups_set()
            self.function_dict['ad'].groups_page().groups_build_switch(_bool)

            self.test_web_login()
            self.function_dict['wp'].main_page().open_user_info()
            self.function_dict['wp'].main_page().check_groups_build(_bool)

    # 測試-新增會員帳號
    # @DecorateClass('CHATAPP-T2508')
    # def test_member_build(self):
    #     self.test_into_and_check_member_list()
    #     self.function_dict['ad'].member_page().build_account(self.member_ID)
    #     build_time = datetime.datetime.now().strftime("%Y/%m/%d %H:%M")
    #     self.function_dict['ad'].member_page().check_data(self.member_ID, build_time, self.revise_time, condition = 'confirm_build')
    ...
    def test_manual_create_account(self):
        ...
        password = '000111abc'
        self.test_into_and_check_member_list()
        self.function_dict['ad'].member_page().manual_create_app_account(self.manual_account_id, phone, password)
        build_time = datetime.datetime.now().strftime("%Y/%m/%d %H:%M")
        self.function_dict['ad'].member_page().check_data(self.manual_account_id, build_time, self.revise_time, condition = 'confirm_manual_create')

        self.function_dict['wp'].base_page().windows_to_top()  # 切換視窗
        self.function_dict['wp'].base_page().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].login_page().login(phone, password, 'CN')
    ...
    def test_member_revise_remark(self):
        ...
        self.function_dict['ad'].member_page().revise_remark(self.manual_account_id)
        self.function_dict['ad'].member_page().check_data(self.manual_account_id, self.build_time, self.revise_time, condition = 'confirm_base')
    ...
    def test_member_change_data(self):
        ...
        self.function_dict['ad'].member_page().member_change_data(self.manual_account_id)
        self.function_dict['ad'].member_page().check_data(self.manual_account_id, self.build_time, self.revise_time, condition = 'confirm_base')
    ...
    def test_member_reset_security_password(self):
        ...
        self.function_dict['ad'].member_page().reset_security_password(self.manual_account_id)
        revise_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        self.function_dict['ad'].member_page().check_data(self.manual_account_id, self.build_time, revise_time, condition = 'confirm_base')
    ...
    def test_member_change_password(self):
        ...
        self.function_dict['ad'].member_page().member_change_password(self.manual_account_id)
        revise_time = datetime.datetime.now().strftime("%Y/%m/%d %H:%M")
        self.function_dict['ad'].member_page().check_data(self.manual_account_id, self.build_time, revise_time, condition = 'confirm_revise')

    # 測試-搜尋功能
    @DecorateClass('CHATAPP-T2515')
    def test_member_search_function(self):
        self.test_into_and_check_member_list()
        self.function_dict['ad'].member_page().search(self.manual_account_id)

    # 測試-刪除後台新增帳號
    @DecorateClass('CHATAPP-T2516')
    def test_member_delete(self):
        self.test_into_and_check_member_list()
        self.function_dict['ad'].member_page().delete_member(self.manual_account_id)

    # @DecorateClass('')
    # def test_change_social_permission(self):
    #     self.test_into_and_check_member_list()
    #     self.function_dict['ad'].member_page().change_social_permission(self.member_ID, enable=False)
    ...
    def test_group_delete(self):
        self.test_admin_login()
        self.function_dict['ad'].main_page().into_groups_list()
        self.function_dict['ad'].groups_page().groups_delete(self.get_group_name())
    ...
    def test_exchange_success_recode(self):
        ...
        self.function_dict['ad'].water_recode_page().search_point_recode(condition='success')
        self.function_dict['ad'].water_recode_page().check_exchange_recode(condition='success')

    # 測試-順付返還積分紀錄
    @DecorateClass('CHATAPP-T2523')
    def test_exchange_fail_recode(self):
        self.test_into_red_integral()
        self.function_dict['ad'].water_recode_page().search_point_recode(condition='fail')
        self.function_dict['ad'].water_recode_page().check_exchange_recode(condition='fail')
    ...
    def test_add_redenvelope(self):
        ...
        self.function_dict['ad'].water_control_page().edit_point(operator, original_water)

        self.function_dict['ad'].main_page().into_red_list()
        self.function_dict['ad'].red_envelope_page().add_redenvelope()
...
# 測試-檢查紅包詳情
    ...
    def test_check_red_envelope(self):
        self.test_into_red_list()
        self.function_dict['ad'].red_envelope_page().redenvelope_detail(60)

    # 測試-檢查拚手氣紅包詳情
    @DecorateClass('CHATAPP-T2520')
    def test_check_luck_red_envelope(self):
        self.test_into_red_list()
        self.function_dict['ad'].red_envelope_page().luck_red_envelope_detail(60)
    ...
    def add_auto_grad_red_envelope(self):
        ...
        self.function_dict['ad'].water_control_page().edit_point(operator, original_water)
        self.function_dict['ad'].main_page().into_red_list()
        self.function_dict['ad'].red_envelope_page().add_red_envelope_for_auto_grad()
    ...
    def add_auto_grad_luck_red_envelope_then_check_water_control(self):
        ...
        operator = 'test1234'
        self.test_into_red_water()
        operateTime = self.function_dict['ad'].water_control_page().edit_point(operator, original_water)  # 回傳操作時間
        self.function_dict['ad'].main_page().into_red_list()
        total_cost = self.function_dict['ad'].red_envelope_page().add_luck_red_envelope_for_auto_grad()  # 回傳使用金額
    ...
    def test_water_control(self, member_name, operate_time, original_water, total_cost, after_water):
        self.test_into_red_water()
        self.function_dict['ad'].water_control_page().test_point_control_search(member_name)
        self.function_dict['ad'].water_control_page().test_water_control_detail(operate_time, member_name, original_water, total_cost, after_water)
    ...
    def test_testtest(self):
        self.function_dict['ad'].base_page().windows_to_top()  # 切換視窗
        self.function_dict['ad'].base_page().open_base_url()  # 開啟前台網站
        self.function_dict['ad'].login_page().login('yorkbackend01', '000111abc')  # 登入admin
        self.function_dict['ad'].groups_page().group_add_members()
        self.function_dict['ad'].groups_page().group_add_members()
