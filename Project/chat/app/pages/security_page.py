...
class SecurityPageLocator:
    ...
    password_button = base.check_device(
        ...
        iOS=base.data_collation(type_kind='name', type_name='股聊密码'),
    )
...
class SecurityPage(Base):
    ...
    def change_password(self, old_pwd, new_pwd):
        ...
        if self.phone_platform.lower() == 'ios':
            ...
            if self.common.poco_exists(SecurityPageLocator.popup_message):
    ...
    def check_value(self, phone, account, nation):

        id_value = self.common.poco_get_text(SecurityPageLocator.id_value)
        phone_value = self.common.poco_get_text(SecurityPageLocator.phone_value)
        ...
        assert phone == phone_value, f'手機號有誤'
        assert account == id_value, f'會員帳號有誤'
