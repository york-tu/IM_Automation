import requests, os, sys, pytz, time
...
class WithdrawManagement(BaseFunction):
   ...
   # 人工出款
   def artificial_withdraw(self, web_account, certification, url, money):
      url = f'{url}apis/manwithdraw'
      ...
      res = self.post(url, data=payload, certification=certification)
      assert res.status_code == 200, f'api狀態碼不正確: {res.status_code}'
      assert res.text.__contains__('已新增'), f'新增人工出款失敗  {res.text}'

      self.sleep(30) # 人工提出會有延遲
