import unittest, string, sys, os, datetime, random, string, re
...
class AdminTestCases(BaseTestCase, BasePageAdmin):
    # Chrome Setting
    wait_time = 20
    ...
    def tearDown(self):
        ...
        if self.wap_browser_switch == True:
            AdminTestCases.wap_browser_switch = False
            self.function_dict['mp'].basePage().quit_browser()
            self.function_dict.pop("mp")
            self.driver_list.pop()
    ...
    @classmethod
    def another_open_wap(cls, wap_browser_switch=True):
        AdminTestCases.wap_browser_switch = wap_browser_switch
        if AdminTestCases.wap_browser_switch == True:
    ...
    def test_web_logout(self):
        self.function_dict["wp"].basePage().windows_to_top() # 切換視窗
        self.function_dict["wp"].mainPage().logout()         # WEB登出
        self.function_dict["wp"].mainPage().logout_checker() # WEB登出檢查
    ...
    def test_admin_logout(self):
        self.function_dict["ad"].basePage().windows_to_top()    # 切換視窗
        self.function_dict["ad"].loginPage().logout()           # ADMIN登出
        self.function_dict["ad"].loginPage().logout_checker()   # ADMIN登出檢查
    ...
    def test_delete_bank_sort(self):
        ...
        # self.function_dict['ad'].Pay_Branch_Page().delete_bank_sort(branch='自動市')
        ...
        self.function_dict['wp'].memberPage().check_company_deposit(bank)  # 確認公司入款有成功刪除
    ...
    def test_add_desposit_wechat_f2f(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_qr_deposit('微信', 'bot', '微信面对面扫码', AdminTestCases.money)  # 確認公司入帳有生成
    ...
    def test_add_desposit_alipay_f2f(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_qr_deposit('支付宝', 'bot', '支付宝面对面扫码', AdminTestCases.money)  # 確認公司入帳有生成
    ...
    def test_add_desposit_wechat_transfer(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().checktr_deposit('微信', '微信轉帳自動測試', '微信转帐', AdminTestCases.money)  # 確認存款類型有生成
    ...
    def test_add_desposit_alipay_transfer(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().checktr_deposit('支付宝', '支付寶轉帳自動測試', '支付宝转帐', AdminTestCases.money)  # 確認存款類型有生成
    ...
    def test_add_desposit_upi(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_company_deposit('UPI入款', 'upi轉帳自動測試')  # 確認公司入帳有生成
    ...
    def test_delete_crypto_wallet_sort(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_company_deposit(crypto)  # 確認USDT錢包地址有成功刪除
    ...
    def test_add_desposit_crypto_bnb_trc(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().checktr_deposit('USDT', address, 'USDT(TRC)', AdminTestCases.money)  # 確認存款類型有生成
    ...
    def test_add_desposit_crypto_bnb_erc(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().checktr_deposit('USDT', address, 'USDT(ERC)', AdminTestCases.money)  # 確認存款類型有生成
    ...
    def test_delete_online_bank_sort(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay(bank)  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_bankscan(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_银行扫码')  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_banktransfer(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_银行转帐')  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_wechatscan(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_微信扫码')  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_wechattransfer(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_微信转帐')  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_alipayscan(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_支付宝扫码')  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_alipaytransfer(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_支付宝转帐')  # 確認存款類型有生成
    ...
    def test_add_online_desposit_kcurrency_scanpay(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付','K币_扫码支付')  # 確認存款類型有生成
    ...
    def test_add_wellpay_bankaccount(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_wellpay_account('顺付WellPay-我要买入款', '顺付WellPay-账号入款','我要买-银行卡账号')  # 確認存款類型有生成
    ...
    def test_add_wellpay_alipayaccount(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_wellpay_account('顺付WellPay-我要买入款', '顺付WellPay-账号入款','我要买-支付宝账号')  # 確認存款類型有生成
    ...
    def test_add_wellpay_wechataccount(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_wellpay_account('顺付WellPay-我要买入款', '顺付WellPay-账号入款','我要买-微信账号')  # 確認存款類型有生成
    ...
    def test_add_wellpay_alipayscan(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_wellpay_account('顺付WellPay-我要买入款', '顺付WellPay-扫码入款','我要买-支付宝扫码')  # 確認存款類型有生成
    ...
    def test_add_wellpay_wechatscan(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_wellpay_account('顺付WellPay-我要买入款', '顺付WellPay-扫码入款','我要买-微信扫码')  # 確認存款類型有生成
    ...
    def test_bank_account_page(self):
        ...
        # admin 取資料
        admin_all_count, admin_today_count, admin_all_number, admin_today_number = self.function_dict['ad'].Pay_Branch_Page().get_withdraw_info(id) # Admin端數值

        # 進入 web端 線上存款 
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()                                       # 進入線上取款
        self.function_dict['wp'].memberPage().check_company_deposit('公司入款', name)                        # 選擇存款方式
        self.function_dict['wp'].memberPage().do_company_deposit('測試', deposit_money)                     # 進行線上存款
        ...
        # admin 取資料
        self.function_dict['ad'].financialmanagementPage().into_bank_account()                                                                                    # 進入銀行帳號
        new_admin_all_count, new_admin_today_count, new_admin_all_number, new_admin_today_number = self.function_dict['ad'].Pay_Branch_Page().get_withdraw_info(id) # Admin端數值

        # 確認info有改
        assert new_admin_all_count == (admin_all_count + 1), "總收款次數錯誤"
        assert new_admin_today_count == (admin_today_count + 1), "當日收款次數錯誤"
        assert new_admin_all_number == (admin_all_number + deposit_money), "總收款總額錯誤"
        assert new_admin_today_number == (admin_today_number + deposit_money), "當日收款總額錯誤"
        ...
        self.function_dict['ad'].Pay_Branch_Page().delete_bank_sort(bank_sort=bank_sort, bank_number=id, status='0')    # 確定刪除功能        
    ...
    def test_online_merchant_page(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_deposit()                                       # 進入線上取款
        self.function_dict['wp'].memberPage().check_onlinepay('线上支付', '线上支付', name)                  # 選擇存款方式
        self.function_dict['wp'].memberPage().do_all_online_deposit(deposit_money, name)                     # 進行線上存款
    ...
    def test_handing_fee_page(self):
        ...
        new_list, error_list = self.function_dict['ad'].HandingFeePage().check_save()               # 確認所有存檔按鈕功能 並取得存檔後之值

        # 進入 web端 取線上取款資料
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_withdraw()                                               # 進入線上取款
        hour, free_time, time, charge, min_data, max_data = self.function_dict['wp'].memberPage().get_online_withdraw_info() # 抓取 線上取款 之值 (進入銀行卡出款確認)

        # 比較 admin 和 web 端之值
        assert hour == str(new_list[0]), "admin {} 與web資料不同".format(error_list[0])
        assert free_time == str(new_list[1]), "admin {} 與web資料不同".format(error_list[1])
        assert min_data == str(new_list[4]), "admin {} 與web資料不同".format(error_list[4])
        assert max_data == str(new_list[5]), "admin {} 與web資料不同".format(error_list[5])
    ...
    def test_period_management_page(self):
        ...
        self.function_dict['wp'].lotteryPage().into_jisuk3()                                                    # 進入急速快3
        bet_period = self.function_dict['wp'].lotteryGamePage().js3_bet()                                   # 下注急速快3三軍的彩種1 並 回傳下注期數
        ...
        # 進入 web端 取得急速快三狀態
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_lottery()                                               # 進入線上取款
        self.function_dict['wp'].lotteryPage().into_jisuk3()                                                    # 進入急速快3
        self.function_dict['wp'].lotteryPage().into_my_order()                                                # 進入投注紀錄
        
        old_value = self.function_dict['wp'].lotteryGamePage().get_js3_bet_result(period=bet_period)
    ...
    def test_handicap_management_page(self):
        ...
        try:
            ...
            self.test_web_login()
            self.function_dict['wp'].menuPage().into_menu_lottery()                                               # 進入線上取款
            self.function_dict['wp'].lotteryPage().into_js6()
            info = self.function_dict['wp'].lotteryGamePage().get_odds_info()

            assert info.__contains__(str(odds_num)), "admin 新增的盤口資訊沒有套用到 web 端"
            ...
            self.function_dict['ad'].operationmanagementPage().into_js6_odds_management() # 進入極速六合彩 -> 盤口管理
            self.function_dict['ad'].OddsManagementPage().modify_odds(odds_origin)
            raise EOFError('admin 新增的盤口資訊沒有套用到 web 端')
    ...
    def test_channel_setting_page(self):
        ...
        # 進入 web端 檢查是否進入維護
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_electronic()                                            # 進入 LGD 電子
        self.function_dict['wp'].electronicPage().maintenance_check(is_maintenance=True)                    # 檢查 LGD 電子是否進入維護
    ...
    def test_different_report(self):
        ...
        artificial_money = self.function_dict['ad'].ArtificialDeposit().get_all_amount_money(self.web_account, '人工公司入款') # 取得轉帳的總計金額
        ...
        self.function_dict['ad'].financialmanagementPage().into_manual_deposit() # 進入 人工存入 頁面
        artificial_money = self.function_dict['ad'].ArtificialDeposit().get_all_amount_money(self.web_account, '人工在线入款') # 取得轉帳的總計金額
    ...
    def test_external_plateform_ledger(self):
        ...
        # assert amount_after - amount_before == difference, f'CmWebPage 廠商錢包 ... 金額顯示錯誤 {amount_after} 應為-> {amount_before + difference}'  #因廠商錢包仍會因為其他變數而改變金額故不比對廠商錢包
        # -------------------------------------------------------- AdminPage --------------------------------------------------------
    ...
    def test_ticket_center_total_check(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_lottery()  # 進入彩票遊戲
        self.function_dict['wp'].lotteryPage().into_jisuk3()       # 進入極速快3
        ...
        # ===========注單中心操作=============
        self.test_admin_login()
        self.function_dict['ad'].operationmanagementPage().into_order_center()                                                                     # 進入運維管理 -> 注單中心
    ...
    def test_payout_statistics(self):
        # ------------------------------------------------ 前台 ------------------------------------------------ 
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_center() # 進入會員專區
        self.function_dict['wp'].memberPage().into_L_MenuBettingRecord() # 進入投注紀錄
        self.sleep(60)
        web_record = self.function_dict['wp'].gameRecordPage().get_game_record_detail()
        ...
        self.function_dict['ad'].payoutstatiscticsPage().into_payout_statistic() # 進入派彩統計頁面
        total_name, payout_list, record = self.function_dict['ad'].payoutstatiscticsPage().check_total_record_by_member(self.web_account, web_record)
    ...
    def test_mine_sweeping_hall_management(self):
        ...
        self.another_open_wap(wap_browser_switch=True)
        data = self.function_dict['mp'].redEnvelopePage().get_bot_hall_data()
        self.function_dict['mp'].basePage().hide_windows()
    ...
    def test_betting_limit_js6(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_js6_bet_limit)

    # 五分快3 -> 投注限額
    def test_betting_limit_k3(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_k3_bet_limit)

    # 五分PK拾 -> 投注限額
    def test_betting_limit_pk10(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_pk10_bet_limit)

    # 五分時時彩 -> 投注限額
    def test_betting_limit_sscb(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_ssc_bet_limit)

    # 五分幸運28 -> 投注限額
    def test_betting_limit_xy28(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_xy28_bet_limit)

    # 三分六合彩 -> 投注限額
    def test_betting_limit_3f6(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_3f6_bet_limit)

    # 香港六合彩 -> 投注限額
    def test_betting_limit_hk6(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_hk6_bet_limit)

    # 福彩3D -> 投注限額
    def test_betting_limit_fc3d(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_fc3d_bet_limit)

    # 排列三 -> 投注限額
    def test_betting_limit_pl3(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_pl3_bet_limit)

    # 分分六合彩 -> 投注限額
    def test_betting_limit_ff6(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_ff6_bet_limit)

    # 極速時時彩 -> 投注限額
    def test_betting_limit_jsssc(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_jsssc_bet_limit)

    # 極速PK拾 -> 投注限額
    def test_betting_limit_jspk10(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_jspk10_bet_limit)

    # 極速快3 -> 投注限額
    def test_betting_limit_jisuk3(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_jisuk3_bet_limit)

    # 幸運飛艇 -> 投注限額
    def test_betting_limit_lucky_airship(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_luckyairship_bet_limit)

    # PC蛋蛋 -> 投注限額
    def test_betting_limit_pc(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_pc_bet_limit)

    # 台灣幸運28 -> 投注限額
    def test_betting_limit_tw(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_tw_bet_limit)

    # 加拿大幸運28 -> 投注限額
    def test_betting_limit_canada(self):
        self.betting_limit(self.function_dict['ad'].operationmanagementPage().into_canada_bet_limit)
    ...
    def test_game_setting_js6(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_js6_game_setting, self.function_dict['wp'].lotteryPage().into_js6, is_maintenance=True)
    
    # 香港六合彩 -> 遊戲設置
    def test_game_setting_hk6(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_hk6_game_setting, self.function_dict['wp'].lotteryPage().into_hk, is_maintenance=True)
    
    # 福彩3D -> 遊戲設置
    def test_game_setting_fc3d(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_fc3d_game_setting, self.function_dict['wp'].lotteryPage().into_fu3d, is_maintenance=True)
    
    # 極速時時彩 -> 遊戲設置
    def test_game_setting_jsssc(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_jsssc_game_setting, self.function_dict['wp'].lotteryPage().into_jsssc, is_maintenance=True)
    
    # 極速PK拾 -> 遊戲設置
    def test_game_setting_jspk10(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_jspk10_game_setting, self.function_dict['wp'].lotteryPage().into_jspk10, is_maintenance=True)
    
    # 極速快3 -> 遊戲設置
    def test_game_setting_jisuk3(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_jisuk3_game_setting, self.function_dict['wp'].lotteryPage().into_jisuk3, is_maintenance=True)
    
    # PC蛋蛋 -> 遊戲設置
    def test_game_setting_pc(self):
        self.game_setting(self.function_dict['ad'].operationmanagementPage().into_pc_game_setting, self.function_dict['wp'].lotteryPage().intoPcegg, is_maintenance=True)
    ...
    # 系統管理 -> 系統設定 -> 網站設定
    @DecorateClass('PFREQ-T2106')
    def test_website_setting_page_signup_to_website_maintain(self):
        ...
        self.function_dict["ad"].WebsiteSetting().member_registered(agent) # 一律先行啟用
        self.sleep(70)
        self.function_dict['ad'].WebsiteSetting().member_registered(agent, setting=False) # 會員註冊_禁用
        self.sleep(70)
        self.test_web_login()
        ...
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].mainPage().into_register_page()        
        self.function_dict['wp'].registerPage().do_register(self.bot_id, '123456', self.brand, '111111', captcha=0)
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting() # 進入網站設定頁
        self.function_dict["ad"].WebsiteSetting().member_registered(agent) # 一律先行啟用
        self.sleep(70)
        ...
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].mainPage().into_register_page()
        self.function_dict['wp'].registerPage().doagent_registered(self.bot_id, '123456', captcha=0)
        # 代理註冊-啟用
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        ...
        self.function_dict['wp'].basePage().windows_to_top()
        self.function_dict['wp'].basePage().refresh_browser()
        self.function_dict['wp'].mainPage().front_web_banned(web_url)
        # 站點維護-啟用
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        ...
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.sleep(1)
        self.function_dict['wp'].memberPage().refresh_browser() # 重新啟用前台網頁後, 會出現一般公告, 刷新去除公告彈窗
        self.function_dict['wp'].mainPage().into_register_page()
        # 站點維護-自動維護
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
    ...
    # 系統管理 -> 系統設定 -> 網站設定 (手機優惠大廳 - 額度轉換顯示設定)
    @DecorateClass('PFREQ-T2377')
    def test_website_setting_page_discounthall_to_timeout(self):
        ...
        self.function_dict['wp'].basePage().windows_to_top()
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].mainPage().firstOpenBaseUrl()
        self.function_dict['wp'].mainPage().check_mobile_hall()
        self.function_dict['wp'].basePage().switch_home_page()
        ...
        self.function_dict['wp'].mainPage().into_register_page()
        self.bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        self.function_dict['wp'].registerPage().check_Register(self.bot_id, self.brand, '123456', '111111', display=0, captcha=0) # 設計為無法註冊, 僅測試個欄位是否可用
        # self.function_dict['wp'].basePage().close_login_board() # 此流程暫定不跑, 暫定註解
        # self.function_dict['wp'].mainPage().logout() # 此流程暫定不跑, 暫定註解
        ...
        self.function_dict['wp'].mainPage().into_register_page()
        self.bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        self.function_dict['wp'].registerPage().check_Register(self.bot_id, self.brand, '123456', '111111', display=2, captcha=0) # 設計為無法註冊, 僅測試個欄位是否可用
        # self.function_dict['wp'].basePage().close_login_board() # 此流程暫定不跑, 暫定註解
        # self.function_dict['wp'].mainPage().logout() # 此流程暫定不跑, 暫定註解
        ...
        self.function_dict['wp'].mainPage().into_register_page()
        self.bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        self.function_dict['wp'].registerPage().check_Register(self.bot_id, self.brand, '123456', '111111', display=1, captcha=0) # 設計為無法註冊, 僅測試個欄位是否可用
        # self.function_dict['wp'].basePage().close_login_board() # 此流程暫定不跑, 暫定註解
        # self.function_dict['wp'].mainPage().logout() # 此流程暫定不跑, 暫定註解
        ...
        self.function_dict['wp'].chessPage().check_transferstatus()
        # 額度轉換框-不顯示
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        ...
        # self.sleep(180)     # cache+等登出
        self.sleep(70)      # cache時間等待
        self.test_admin_logout()
        self.sleep(3)
        self.test_admin_login()
        start_time = self.function_dict['ad'].WebsiteSetting().counter_time()
        self.sleep(60)
        self.function_dict['ad'].basePage().refresh_browser()
        self.function_dict['ad'].loginPage().logout_checker()
        end_time = self.function_dict['ad'].WebsiteSetting().counter_time()
        self.function_dict['ad'].WebsiteSetting().auto_logout_time_check(start_time=start_time, end_time=end_time)
        self.sleep(3)
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
    ...
    # 系統管理 -> 系統設定 -> 網站設定 (在線客服管理)
    @DecorateClass('PFREQ-T2378')
    def test_website_setting_page_online_customer_manage(self):
        ...
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].mainPage().firstOpenBaseUrl()
        self.function_dict['wp'].mainPage().check_qq_link(self.brand) 

        # # 在線客服 已全面使用客服寶，現在此功能無作用
        # self.test_admin_login()
        # self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        ...
        # self.sleep(120)
        # self.function_dict['wp'].basePage().refresh_browser()
        # self.function_dict['wp'].mainPage().check_livechat_link(self.brand, cs=option)
        # self.function_dict['wp'].basePage().switch_home_page()
        # self.test_admin_login()
        # self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        ...
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].mainPage().firstOpenBaseUrl()
        self.function_dict['wp'].mainPage().check_email_link(self.brand)
        ...
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        initial_url = self.function_dict['ad'].WebsiteSetting().customerservice_get_value()
        self.function_dict['ad'].WebsiteSetting().customerservice_setting()
        self.function_dict['ad'].MarketCenterPage().into_float_window_management()
        ...
        self.function_dict['wp'].mainPage().firstOpenBaseUrl()
        self.function_dict['wp'].mainPage().check_customerservice_link()
        web_url = self.function_dict["wp"].basePage().get_url()  
        self.function_dict['wp'].basePage().switch_home_page()

        # 站內客服reset_右側浮窗&站內客服網址
        self.test_admin_login()
        self.function_dict['ad'].MarketCenterPage().into_float_window_management()
        self.function_dict['ad'].FloatWindows().right_revising_reset()
        self.function_dict["ad"].FloatWindows().right_floating_reset()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict['ad'].WebsiteSetting().customerservice_setting(initial_url)
        self.sleep(70)

        # APP下載框
        if self.brand != 'xpj':     # 吃前端設定檔 xpj 已許久不顯示
            self.test_admin_login()
            self.function_dict['ad'].systemmanagementpage().into_admin_setting()
            ...
            self.function_dict['wp'].mainPage().firstOpenBaseUrl()
            self.sleep(2)
            self.function_dict['wp'].mainPage().check_app_download()
    ...
    def test_online_deposit(self):
        ...
        self.function_dict['ad'].basePage().check_page_v2() # 判斷頁數及尾頁餘數數量正確
        # self.function_dict['ad'].OnlineDepositPage().check_export_report(self.brand) # 匯出測試
    ...
    def test_member_wallet(self):
        ...
        self.function_dict['wp'].memberPage().do_wallet_conversion('cp', 'ag', 1)
        wallet_list_front = self.function_dict['wp'].memberPage().get_name_and_money()
    ...
    def test_withdraw_page(self):        
        ...
        self.function_dict['wp'].memberPage().do_change_bankCard('台灣', '台北', self.withdraw_password)  # 設定銀行開戶網點
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        self.function_dict['wp'].memberPage().refresh_browser()  # 關閉一般公告 
        for _ in range(0,3):
            self.function_dict['wp'].menuPage().into_menu_member_withdraw()  # 進入線上取款
            self.function_dict['wp'].memberPage().do_withdraw(self.money, self.withdraw_password)  # 取款
            self.sleep(10)        
    ...
    def test_cgpay_withdraw_page(self):   
        ...
        for _ in range(0,3):   
            self.function_dict['wp'].menuPage().into_menu_member_withdraw()  # 進入線上取款         
            self.function_dict['wp'].memberPage().do_cgpay_withdraw(self.money, self.withdraw_password)  # CGpay取款
            self.sleep(10)        
    ...
    def test_company_deposit_otp(self):
        self.test_admin_login()
        # ===========otp開啟=============
        ...
        self.function_dict['wp'].menuPage().into_menu_member_deposit()  # 進入線上存款
        self.function_dict['wp'].memberPage().check_company_deposit('公司入款', '銀行自動測試')  # 確認公司入帳有生成
        self.function_dict['wp'].memberPage().do_company_deposit('銀行自動測試', self.money, otp_code)
    ...
    def test_desposit_alipay_f2f_otp(self):
        self.test_admin_login()
        # ===========otp開啟=============
        self.function_dict['ad'].membermanagementpage().into_level_management() # 進入層級管理
        self.function_dict['ad'].levelmanagementPage().turn_on_deposit_otp()  # 開啟OTP
        ...
        self.function_dict['wp'].memberPage().do_desposit_weChat_alipayf2f(otp_code) # 支付寶/微信 面對面
        self.function_dict['wp'].memberPage().before_check_money_entry_record('公司入款', self.money) # 確認產生充值中紀錄
        # ADMIN PART
        self.test_admin_login()
        self.function_dict['ad'].financialmanagementPage().into_company_deposit()  # 進入公司入款
    ...
    def test_desposit_wechat_transfer_otp(self):
        self.test_admin_login()
        # ===========otp開啟=============
        self.function_dict['ad'].membermanagementpage().into_level_management() # 進入層級管理
        self.function_dict['ad'].levelmanagementPage().turn_on_deposit_otp()  # 開啟OTP
        ...
        self.function_dict['wp'].memberPage().do_desposit_wechat_alipay_transfer(otp_code)
    ...
    def test_manwithdraw_page(self):       
        ...
        self.function_dict['ad'].ManwithdrawPage().do_all_option(self.web_account)    # 新增所有人工提出的項目
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_wallet()
        wallet_before = ('%.2f' %self.function_dict['wp'].mainPage().get_web_wallet())                       
    ...
    def test_withdrawgeneral_page(self):
        self.test_admin_login()
        self.function_dict['ad'].financialmanagementPage().into_withdraw_general() # 進入下分總覽
        ...
        self.function_dict['ad'].withdrawGeneralPage().check_shareholder_generalagent_agent(self.shareholder, self.generalagent, self.reseller_account) # 股東/總代/代理 #lv股東太多導致下拉選單沒出現bot的股東，已開單處理 PFREQ-1985
        ...
        self.function_dict['ad'].withdrawGeneralPage().detail_manpower(self.web_account) # 人工提出明細
    ...
    def test_realtime_lottery_page(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_member_center()                                                                               
        self.function_dict['wp'].memberPage().into_L_MenuBettingRecord()
        web_record = self.function_dict['wp'].gameRecordPage().get_game_record_detail()
        ...
        self.function_dict['ad'].RealTimeOrderPage().history_period_action(web_record)
        self.function_dict['ad'].RealTimeOrderPage().sort_check_action(web_record)
        
    # 下注頁面統一使用(先以api test_lottery_bet_order取代)
    @DecorateClass('PFREQ-T2059')
    def test_bet_lottery(self):
        # =========前台下注=========
        self.test_web_login()
        lottery_record_list = []
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_hk()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().betting_hk))
        self.function_dict['wp'].basePage().switch_home_page()
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_jsssc()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().betting_jsssc))
        self.function_dict['wp'].basePage().switch_home_page()
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_jspk10()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().betting_jspk10))
        self.function_dict['wp'].basePage().switch_home_page()
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_pcegg()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().pcegg))
        self.function_dict['wp'].basePage().switch_home_page()
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_jisu11to5()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().jisu11to5))
        self.function_dict['wp'].basePage().switch_home_page()
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_jisuk3()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().js3))
        self.function_dict['wp'].basePage().switch_home_page()
        self.function_dict['wp'].menuPage().into_menu_lottery()
        self.function_dict['wp'].lotteryPage().into_fu3d()
        lottery_record_list.append(self.function_dict['wp'].lotteryGamePage().betting_game(self.function_dict['wp'].lotteryGamePage().fu3d))
        self.function_dict['wp'].basePage().switch_home_page()
    ...
    def test_artificial_deposit_page(self):
        # ------------------------------------------------ 前台動作 ------------------------------------------------
        self.test_web_login()
        wallet_balance = self.function_dict['wp'].menuPage().get_balance_wallet()
        # ------------------------------------------------ 後台動作 ------------------------------------------------
        self.test_admin_login()
        ...
        self.function_dict['wp'].mainPage().sleep(1)
        new_wallet = self.function_dict['wp'].menuPage().get_balance_wallet()
        # 比較前台金額是否成功添加
    ...
    def test_financial_report_page(self):
        ...
        self.function_dict['ad'].FinancialReportPage().search_for_agent(self.reseller_account)     # 查找股東
        data_list = self.function_dict['ad'].FinancialReportPage().get_data()               # 財務報表數據
        self.function_dict['ad'].FinancialReportPage().verify_detail_data(data_list)
    ...
    # 網站設定 > WEB/ WAP_會員 & 代理註冊開啟/ 禁用
    @DecorateClass('PFREQ-T6089')
    def test_sign_up(self):
        # 會員註冊 & 代理註冊
        bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        agent = "default_agent"
        self.test_admin_login()
        self.function_dict["ad"].systemmanagementpage().into_admin_setting()
        self.function_dict["ad"].WebsiteSetting().member_registered(agent)  # 一律先行啟用會員註冊功能
        self.function_dict['ad'].WebsiteSetting().agent_registered(self.brand)  # 一律先行啟用代理註冊功能
        self.sleep(60)
        self.function_dict['ad'].WebsiteSetting().member_registered(agent, setting=False)   # 開始進行會員註冊禁用
        self.sleep(60)
        self.test_web_login()
        self.function_dict["wp"].mainPage().into_register_page()
        self.function_dict["ad"].basePage().windows_to_top()
        self.function_dict["ad"].WebsiteSetting().member_registered(agent)
        self.function_dict['ad'].WebsiteSetting().agent_registered(self.brand, setting=False)   # 開始進行代理註冊禁用
        self.sleep(60)
        self.test_web_login()
        self.function_dict["wp"].mainPage().into_register_page()
        self.function_dict["wp"].registerPage().doagent_registered(bot_id, "ps123456", self.brand, captcha=0)
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        # 開始進行會員註冊啟用      
        self.function_dict['ad'].WebsiteSetting().member_registered(agent)
        ...
        self.refresh_browser()
        self.function_dict['wp'].mainPage().into_register_page()        
        self.function_dict['wp'].registerPage().do_register(bot_id, 'ps123456', self.brand, '111111', captcha=0)
        self.function_dict["wp"].basePage().open_base_url()
        self.refresh_browser()
        self.function_dict["wp"].mainPage().into_register_page()
        self.function_dict["wp"].registerPage().doagent_registered(bot_id, "ps123456", self.brand, captcha=0)
    ...
    # 網站設定 > 試玩 & 聯盟 & 滑動驗證_啟用/ 禁用
    @DecorateClass('PFREQ-T6179')
    def test_try_verification(self):
        # 會員試玩-禁用
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict['ad'].WebsiteSetting().memeber_play_setting(setting=False)
        ...
        self.function_dict['ad'].basePage().open_base_url()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict["ad"].WebsiteSetting().slide_verified(setting=False)
        self.sleep(60)
        status = self.test_web_login()
        self.function_dict["ad"].WebsiteSetting().slide_confirm(target=True, status=status)
        self.test_web_logout()
        # 滑動驗證-啟用
        self.function_dict["ad"].basePage().windows_to_top()
        self.function_dict['ad'].basePage().open_base_url()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict["ad"].WebsiteSetting().slide_verified(setting=True)
        self.sleep(60)
        status = self.test_web_login()
        self.function_dict["ad"].WebsiteSetting().slide_confirm(target=False, status=status)
        self.test_web_logout()

    # 網站設定 > 站點維護
    @DecorateClass('PFREQ-T6180')
    ...
    # 網站設定 > 手機優惠大廳
    @DecorateClass('PFREQ-T6193')
    def test_phone_discountlobby(self):
        # 先行測試已有的左側浮窗開關流程
        self.test_web_login()
        self.function_dict["wp"].mainPage().left_float_windows_action()
        # 手機優惠大廳
        ...
        self.function_dict['ad'].WebsiteSetting().mobile_hall()
        self.function_dict['ad'].MarketCenterPage().into_float_window_management()
        # self.function_dict['ad'].FloatWindows().into_left_revising()    # 左側浮窗修改_狀態顯示、上傳圖片
        # self.function_dict['ad'].FloatWindows().mobile_hall_link()      # 左側浮窗_設定優惠大廳
        self.function_dict['ad'].FloatWindows().left_floating_add_promo()
        self.sleep(150)
        self.test_all_windows_mini()
        self.function_dict['wp'].basePage().windows_to_top()
        self.function_dict['wp'].basePage().open_base_url()  # 開啟前台網站
        ...
        self.function_dict["ad"].FloatWindows().left_floating_reset()
        self.function_dict['ad'].MarketCenterPage().into_float_window_management()
        self.function_dict['ad'].FloatWindows().close_floatwindows_link()

    # 網站設定 > 註冊細節
    @DecorateClass('PFREQ-T6205')
    def test_signup_detail(self):
        ...
        self.function_dict['wp'].mainPage().into_register_page()
        self.bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        self.function_dict['wp'].registerPage().check_Register(self.bot_id, self.brand, 'ps123456', '111111', display=0, captcha=0) # 設計為無法註冊, 僅測試個欄位是否可用
        # self.function_dict['wp'].basePage().close_login_board() # 此流程暫定不跑, 暫定註解
        # self.function_dict['wp'].mainPage().logout() # 此流程暫定不跑, 暫定註解
        # 手機/郵箱/QQ號碼/微信/推薦人設定-必填
        self.test_admin_login()
        ...
        self.function_dict['wp'].mainPage().into_register_page()
        self.bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        self.function_dict['wp'].registerPage().check_Register(self.bot_id, self.brand, 'ps123456', '111111', display=2, captcha=0) # 設計為無法註冊, 僅測試個欄位是否可用
        # self.function_dict['wp'].basePage().close_login_board() # 此流程暫定不跑, 暫定註解
        # self.function_dict['wp'].mainPage().logout() # 此流程暫定不跑, 暫定註解
        # 手機/郵箱/QQ號碼/微信/推薦人設定-選填
        self.test_admin_login()
        ...
        self.function_dict['wp'].mainPage().into_register_page()
        self.bot_id = datetime.datetime.now().strftime('bot%m%d%S')
        self.function_dict['wp'].registerPage().check_Register(self.bot_id, self.brand, 'ps123456', '111111', display=1, captcha=0) # 設計為無法註冊, 僅測試個欄位是否可用
        # self.function_dict['wp'].basePage().close_login_board() # 此流程暫定不跑, 暫定註解
        # self.function_dict['wp'].mainPage().logout() # 此流程暫定不跑, 暫定註解

    # 網站設定 > 額度轉換顯示
    @DecorateClass('PFREQ-T6207')
    def test_quota_conversion(self):
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_game()
        wallet = self.function_dict['wp'].chessPage().check_transferstatus(setting=0)
        # 額度轉換框-不顯示
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict['ad'].WebsiteSetting().transfer_status(setting=1)
        ...
        self.test_web_login()
        self.function_dict['wp'].menuPage().into_menu_game()
        self.function_dict['wp'].chessPage().check_transferstatus(setting=1)
        # 額度轉換框-显示 当金额少於 $
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict['ad'].WebsiteSetting().transfer_status(setting=2)
        ...
        if wallet >= 100:
            ...
            self.function_dict['wp'].chessPage().check_transferstatus(setting=0)

    # 網站設定 > 會員預警 & 超時登出
    @DecorateClass('PFREQ-T6209')
    def test_member_system(self):
        # 會員預警(會通知於slack)
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict['ad'].WebsiteSetting().member_warning(self.brand)
    ...
    # 系統管理 -> 系統設定 -> 網站設定 (在線客服管理)
    @DecorateClass('PFREQ-T6210')
    def test_customer_service_manage(self):
        # 微信二維碼/提示文案
        self.test_admin_login()
        self.function_dict['ad'].systemmanagementpage().into_admin_setting()
        self.function_dict['ad'].WebsiteSetting().wechat_setting()
    ...
    # 系統管理 -> 系統設定 -> 網站設定 (在線客服管理_側邊浮窗測試)
    @DecorateClass('PFREQ-T6211')
    def test_sidefloat_transmit(self):
        # 先行測試已有的右側浮窗開關流程
        self.test_web_login()
        self.function_dict["wp"].mainPage().right_float_windows_action()
        # 站內客服
    ...
    # 將浮窗最後一個連結預設為收起浮窗
    def test_initialize_last_floating(self):
        self.test_admin_login()
        self.function_dict['ad'].MarketCenterPage().into_float_window_management()
        self.function_dict['ad'].FloatWindows().close_floatwindows_link()
