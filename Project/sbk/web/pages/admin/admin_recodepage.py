...
class RecodePageLocator:
    # LOADING
    loading_mask = (By.XPATH, '//div[@class="el-loading-mask"]')

class RecodePage(BasePage):
    def wait_loading_finish(self):
        self.sleep(1)
        if self.is_element_finded(RecodePageLocator.loading_mask) is True:
            try:
                self.is_element_displayed(RecodePageLocator.loading_mask)
            except:
